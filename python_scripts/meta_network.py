...
def build_network_prokka():
    ...
    for algue in  list_algue:
        ...
        sub.run([f'mkdir /scratch/clucas/sbml_prokka/{algue.split("/")[-1]}'], shell=True)
        ...
        if len(bacts) > 1:
            ...
            sub.run([f'singularity exec -B /scratch/clucas:/scratch/clucas /scratch/clucas/m2m_c.sif m2m recon --noorphan --clean -g {genome} -o {output} -c {cpu} '], shell = True)

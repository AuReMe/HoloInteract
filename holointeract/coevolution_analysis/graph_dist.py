...
def get_graph_csv(matrix_file, output: str, phylogenetic_tree: str):
    ...
    with open(f'{output}.csv', 'w') as o:
        ...
        for comm, host_data in df.iterrows():
            comm_host = str(comm).split('_')[0]
            for host, value in host_data.iteritems():
                if host == comm_host:
                    distance = 0
                else:
                    distance = phylo_dist[comm_host][str(host)]
                o.write('\t'.join([str(host), str(comm), str(value), str(distance)]) + '\n')
